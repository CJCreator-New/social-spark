...
- [x] P5 — Run `npx playwright install` locally to fix the firefox/webkit version mismatch — done; full 5-browser suite now passes (see P5 resolution note for what that surfaced)
...
**Resolution:** Resolved. You ran `npx playwright install` locally; the full cross-browser suite (chromium/firefox/webkit/mobile-chrome/mobile-safari, 155 tests) then ran cleanly. Getting there surfaced 6 more real, previously-invisible issues — one genuine app bug and five test-infrastructure gaps that were producing non-deterministic failures specifically on the browser engines that hadn't been exercised before this pass:
- **Real app bug — `--text3` cascade collision.** `contentforge.css` and `Index.css` both define `--text3` on the identical `.cf-app` selector (equal specificity); `Index.css` already had the earlier session's fix (`#5a5753`) but `contentforge.css` still had the original broken `#78716c`. Whichever file's rule landed later in the bundle won the cascade tie, so which value was "active" depended on Vite's chunk/import order for a given route — genuinely non-deterministic in production, not just in tests. Fixed by aligning `contentforge.css`'s value to match.
- **`.ld-w-step-num-ghost` (landing page "01"/"02"/"03" background numerals)** — 1.12:1 contrast, `aria-hidden`, purely decorative behind each "How it works" step card whose real content has its own properly-contrasted text. This is WCAG 1.4.3's "incidental" exception (decorative text has no contrast minimum); axe can't infer that, so excluded it from the scan via `axe.run()`'s `context.exclude` rather than raising the numeral's opacity and ruining the intended subtle-watermark effect.
- **GSAP/framer-motion entrance animations raced the accessibility and calendar-creation e2e tests.** `LandingHero.tsx`'s GSAP timeline and `Index.tsx`'s wizard transitions (already gated on `prefers-reduced-motion` from earlier in this pass) were sometimes still catching axe scans or clicks mid-fade, especially on Firefox/WebKit. Added `test.use({ reducedMotion: "reduce" })` to both `accessibility.spec.ts` and `critical-paths.spec.ts`. The landing page needed one more layer: Playwright's `reducedMotion` context emulation can itself land a beat after LandingHero's very first synchronous `matchMedia()` check on mount (a cross-engine timing gap, worse on Firefox/WebKit), so also added a bounded 2.6s settle wait tied to the widget's own documented worst-case animation budget (1.5s stagger delay + ~0.9s stagger duration) rather than depend on emulation timing alone.
- **`WelcomeBanner.tsx` mounts asynchronously** (gated on `useSubscription()`'s `loading` state) and, once it appears, pushes the industry-selection cards down a real amount — a genuine layout shift, not an animation, so `reducedMotion` didn't touch it. Clicking before it settled landed on stale pre-shift coordinates. Fixed with a self-healing `clickAndConfirmChecked()` helper in `critical-paths.spec.ts` that re-clicks (recomputing coordinates fresh) until the click actually registers, rather than a blind pre-wait.
- **WebKit "Frame load interrupted" on `responsive.spec.ts`'s multi-route navigation loop.** The default `page.goto()` `waitUntil: 'load'` is stricter than needed for these lazy-loaded routes and WebKit intermittently aborted navigation when the previous route's async chunk/font fetches were still in flight. Switched to `waitUntil: "domcontentloaded"`, matching the pattern already used elsewhere in the suite.
- **Lazy-route CSS chunk load race in `accessibility.spec.ts`.** A route's heading becoming visible doesn't guarantee its code-split CSS chunk has finished applying — intermittently caught elements on stale/inherited colors. Added a `page.waitForLoadState("networkidle")` wait in the shared `checkRoute()` helper before invoking axe.

All fixes verified with repeated isolated reruns (10+ repetitions per affected browser/test) plus two full 155-test suite passes: **155/155 clean at `--workers=1`**, **154/155 at `--workers=2`** (the one remaining failure — `webkit` / responsive overflow — reproduces only under `--workers=2` contention and passed 12/12 in isolation, matching the same already-documented CPU-contention-noise class as the original chromium-only pass, not a new defect).
...
The live Playwright pass has now run cleanly across all 5 browser projects (chromium, firefox, webkit, mobile-chrome, mobile-safari), closing the gap from the earlier static-only and chromium-only passes. Getting a clean cross-browser signal surfaced one more genuine app defect (the `--text3` cascade collision, see P5) plus five test-infrastructure timing gaps that were producing non-deterministic failures specifically on the engines that hadn't been exercised before. **Final verification:** `npm run test:run` — 198/198 Vitest tests pass. `npx tsc --noEmit` — clean. `npx playwright test` (all 5 projects, 155 tests) — **155/155 pass at `--workers=1`**; **154/155 at `--workers=2`**, with the one remaining failure (webkit / responsive overflow) reproducing only under `--workers=2` CPU contention and passing 12/12 in isolation — the same already-documented dev-machine/test-infra noise class as before, not a new defect.

Still not covered by any pass to date: a full visual/contrast sweep at both viewports across every remaining route (only `/`, `/auth`, `/app`, `/my-calendars`, `/schedule`, `/calendar/:id` were live-verified), and the deep functional traces listed under F2's original scope (auth flow internals, wizard autosave/recovery, BYOK client-side quota UX, admin dashboard CRUD). Recommend picking those up in a follow-up.
