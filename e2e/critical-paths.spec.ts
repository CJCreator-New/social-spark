...
async function waitForWizardReady(page: Page) {
  await page
    .getByRole("note", { name: /welcome to social spark/i })
...
test.describe("Calendar Creation Flow", () => {
  ...
  test("should create a full week calendar", async ({ page }) => {
    ...
    await page
      .getByPlaceholder(/what's the big idea/i)
      .fill("Launch a better B2B content workflow");
...
test.describe("Calendar Management", () => {
  ...
  test("should bridge from an existing calendar into the Schedule page with its posts visible", async ({
    page,
  }) => {
    await expect(page.getByText("E2E Marketing Launch Week")).toBeVisible({ timeout: 15000 });
...
test.describe("Schedule Management", () => {
  ...
  test("should edit a scheduled post's date/time from the Schedule page and persist the change", async ({
    page,
  }) => {
    // Open the actions menu for the first row and start a reschedule.
    await page
      .getByRole("button", { name: /actions for day/i })
      .first()
      .click();
...
test.describe("Error Handling", () => {
  ...
  test("should handle network errors gracefully", async ({ page }) => {
    ...
    await expect(page.locator(".err-box").first()).toHaveText(
      /connection error\. please check your internet and try again\./i
    );
  });
});
