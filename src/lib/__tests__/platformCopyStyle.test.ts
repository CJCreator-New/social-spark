...
describe('platformCopy style integration', () => {
  it('applies unicode style when options.style is provided', () => {
    const post = { title: '', hook: 'Hello', body: 'Alpha 123', cta: 'Go', hashtags: '' } satisfies Parameters<typeof formatForPlatform>[0]
    const plain = formatForPlatform(post, 'LinkedIn')
    const styled = formatForPlatform(post, 'LinkedIn', { style: FontStyle.Monospace })
