...
function platformBadgeStyle(platform?: string | null): React.CSSProperties {
  ...
  if (p.includes("linkedin"))
    return {
      background: "var(--color-platform-linkedin-bg)",
      color: "var(--color-platform-linkedin-text)",
      border: "1px solid var(--color-platform-linkedin-border)",
    };
  if (p.includes("twitter") || p.includes("x"))
    return {
      background: "var(--color-platform-twitter-bg)",
      color: "var(--color-platform-twitter-text)",
      border: "1px solid var(--color-platform-twitter-border)",
    };
  if (p.includes("instagram"))
    return {
      background: "var(--color-platform-instagram-bg)",
      color: "var(--color-platform-instagram-text)",
      border: "1px solid var(--color-platform-instagram-border)",
    };
  if (p.includes("facebook"))
    return {
      background: "var(--color-platform-facebook-bg)",
      color: "var(--color-platform-facebook-text)",
      border: "1px solid var(--color-platform-facebook-border)",
    };
  if (p.includes("newsletter") || p.includes("blog"))
    return {
      background: "var(--color-platform-generic-bg)",
      color: "var(--color-platform-generic-text)",
      border: "1px solid var(--color-platform-generic-border)",
    };
  return {
    background: "var(--color-surface-muted)",
    color: "var(--color-text-secondary)",
...
export const CalendarItem = React.memo(function CalendarItem({
  ...
  return (
    <motion.div
      ...
      onClick={() => {
        if (!isRenaming) navigate(`/calendar/${it.id}`);
      }}
    ...
    >
          ...
          <div className="flex items-center gap-1" ref={menuRef}>
            ...
            <button
              ...
              style={{
                ...
                display: "flex",
                alignItems: "center",
                color: it.is_favorite ? "var(--color-warning)" : "var(--color-text-disabled)",
            ...
            >
              <Star
                size={15}
                style={{ fill: it.is_favorite ? "var(--color-warning)" : "none", strokeWidth: 1.5 }}
              />
            </button>
            ...
            {menuOpen && (
              ...
              >
                {[
                  ...
                  {
                    label: duplicatingId === it.id ? "Copying…" : "Duplicate",
                    action: () => {
                      setMenuOpen(false);
                      duplicate(it);
                    },
      ...
      >
        <div className="flex gap-2 text-[11px]" style={{ color: "var(--color-text-secondary)" }}>
