...
const POSTED: Set<string> = new Set([
  "0-0",
  "0-2",
  "0-4",
  "1-1",
...
function FeatureA() {
  ...
  return (
    <div ref={ref} className="ld-w-feature-visual" role="img" aria-label="AI writing engine demo">
      ...
      <div className="ld-w-code-arrow" aria-hidden="true">
        ↓ ContentForge generates
      </div>
      <div ref={outputRef} className="ld-w-post-output" style={{ opacity: 0 }}>
        <strong>Hook:</strong> Ethiopian single-origin. 72-hour fermentation. The cup that changed
        how we source.
        <br />
        <br />
        <strong>Body:</strong> We've partnered directly with Haile Selassie Farm in Yirgacheffe to
        bring you a naturally processed bean that carries notes of blueberry, jasmine, and dark
        chocolate.
        <br />
        <br />
  ...
  return (
    <div className="ld-w-feature-visual" role="img" aria-label="Multi-platform scheduling calendar">
      ...
      <div className="ld-w-cal-grid" aria-hidden="true">
        ...
        {DAYS.map((d) => (
          <div key={d} className="ld-w-cal-head">
            {d}
          </div>
        ))}
      ...
      onEnter: () => {
        counterEls.forEach((el, i) => {
          ...
          gsap.to(obj, {
            ...
            ease: "power2.out",
            onUpdate: () => {
              const rounded = Number.isInteger(stat.value)
                ? Math.round(obj.val)
                : obj.val.toFixed(1);
...
const FEATURES = [
  {
    ...
    bullets: [
      { strong: "Voice calibration", text: " — upload past content or describe your style." },
      {
        ...
        text: " — tone adjusts for LinkedIn vs Instagram automatically.",
      },
      {
        strong: "No markdown in copy",
        text: " — clean output, every time. No **bold** artifacts.",
      },
    ],
    visual: <FeatureA />,
    reverse: false,
  },
  {
    ...
    bullets: [
      ...
      {
        strong: "Optimal timing hints",
        text: " — AI suggests the best days and times per platform.",
      },
            ...
            <div className={`ld-w-feature-grid${feat.reverse ? " reverse" : ""}`}>
              <div className="ld-w-feature-copy">
                ...
                <h2 id={`feat-${feat.id}-heading`} className="ld-w-feature-h2">
                  {feat.title.replace(feat.titleEm, "").trim()} <em>{feat.titleEm}</em>
                </h2>
