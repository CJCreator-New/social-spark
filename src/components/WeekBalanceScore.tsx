...
function computeWeekBalance(posts: Post[]): BalanceResult {
  if (posts.length === 0)
    return {
      score: 100,
      label: "Excellent",
      color: WARM_PALETTE.scoreHigh,
      barColor: WARM_PALETTE.scoreHigh,
      warnings: [],
    };
  ...
  if (uniqueFormats.size === 1 && posts.length >= 3) {
    warnings.push({
      ...
      suggestion:
        "💡 Try changing one of the posts to a 'Case Study' or 'How-to Guide' to add format variety.",
    });
    deductions += 25;
    ...
    warnings.push({
      ...
      suggestion:
        "💡 Introduce a 'Behind-the-scenes' or 'Myth-busting' format for one of the days.",
    });
    deductions += 12;
  }
  ...
  if (maxConsec >= 3) {
    warnings.push({
      ...
      suggestion:
        "💡 Alternate between text-heavy narratives and punchy list breakdowns to keep readers engaged.",
    });
  ...
  if (uniqueCtas.size <= 1 && posts.length >= 3) {
    warnings.push({
      ...
      message:
        "CTAs look similar across posts — vary your calls-to-action for better engagement spread.",
      suggestion:
        "💡 Use 'Ask a question' to boost comments on one day, and 'Link to resource' to drive clicks on another.",
    });
    deductions += 10;
  }
  ...
  if (score >= 80) {
    ...
    label = "Fair";
    color = WARM_PALETTE.scoreMed;
    barColor = WARM_PALETTE.scoreMed;
  } else {
    label = "Needs Work";
    color = WARM_PALETTE.scoreLow;
    barColor = WARM_PALETTE.scoreLow;
  }
