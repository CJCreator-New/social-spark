      ...
      {templates.map((t) => (
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button className="pf-add-btn" onClick={() => handleLoad(t)}>
              Load
            </button>
            <button className="pf-add-btn" onClick={() => setPendingDeleteId(t.id)}>
              Delete
            </button>
          </div>
        </div>
...
export default function Profile() {
  ...
  const activeTab =
    (searchParams.get("tab") as "profile" | "brand" | "api-keys" | "plan") || "profile";
  ...
  const activeDefaults =
    [defaultVoice, defaultStyle, defaultTimezone].filter(Boolean).length +
    (defaultAudiences.length > 0 ? 1 : 0) +
    (defaultGoals.length > 0 ? 1 : 0);
  ...
  return (
    <>
      ...
      <WorkspacePage size="narrow">
        ...
        >
          <h1 className="pf-title" style={{ margin: 0, color: "var(--color-text)" }}>
            Profile &amp; Brand
          </h1>
          <TierBadge />
        </div>
        <div className="pf-sub" style={{ color: "var(--color-text-secondary)" }}>
          Update how you appear inside ContentForge and set brand defaults to pre-fill the wizard.
        </div>
        ...
        <div className="pf-summary">
          <div className="pf-summary-card">
            ...
            <div className="pf-summary-sub">
              Profile fields that will pre-fill the next calendar.
            </div>
          </div>
          <div className="pf-summary-card">
          ...
          <div className="pf-summary-card">
            ...
            <div className="pf-summary-value">
              {bannedHashtags.length + requiredHashtags.length}
            </div>
          ...
          <div className="pf-card">
            {profileLoading ? (
              ...
              <>
                ...
                <label className="pf-label" htmlFor="pf-tz">
                  Default timezone
                </label>
                <select
                  id="pf-tz"
        ...
        {activeTab === "brand" && !profileLoading && (
          <div className="pf-card">
            ...
            <div className="pf-section-sub">
              Pre-fill the wizard with your usual voice, style, audiences, and goals. You can still
            ...
            <label className="pf-label" htmlFor="pf-style">
            ...
            <div className="pf-label" id="pf-aud-label">
              Default audiences (up to 6)
            </div>
            ...
            <div className="pf-add-row">
              ...
              />
              <button className="pf-add-btn" onClick={addAudience}>
                Add
              </button>
            </div>

            <div className="pf-label" id="pf-goals-label">
              Default goals
            </div>
            ...
            <div className="pf-label">
              Brand example posts (optional — up to 20; the most relevant ones are auto-selected per
              generation)
            </div>
            <div className="pf-tagrow" aria-hidden>
              {brandExamples.length === 0 ? (
                <div className="pf-tagrow-empty">No examples saved</div>
              ) : (
                brandExamples.map((b, i) => (
                  <div
                    key={i}
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() => setBrandExamples((prev) => prev.filter((_, idx) => idx !== i))}
                    >
                      Remove
                    </button>
                  </div>
            ...
            <div className="pf-add-row">
              <input
                ...
                value={brandExampleInput}
                onChange={(e) => setBrandExampleInput(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addBrandExample())}
              />
              <button className="pf-add-btn" onClick={() => addBrandExample()}>
                Add
              </button>
            </div>
            ...
            <div className="pf-section-sub">
              Lock specific phrases, proofs, CTAs, or formats across all generated content to
              maintain brand consistency.
            </div>

            {/* Forbidden Phrases */}
            <div className="pf-label" id="pf-forbidden-label">
              ...
              {forbiddenPhrases.length === 0 ? (
                ...
                forbiddenPhrases.map((p) => (
                  ...
                  >
                    ...
                    <button
                      className="pf-tag-x"
                      onClick={() => setForbiddenPhrases((prev) => prev.filter((x) => x !== p))}
                      aria-label={`Remove ${p}`}
                      style={{
                        color: "color-mix(in srgb, var(--color-error-text) 50%, transparent)",
                      }}
                    >
                      ×
                    </button>
            ...
            <div className="pf-add-row">
              ...
              />
              <button className="pf-add-btn" onClick={addForbiddenPhrase}>
                Add
              </button>
            </div>

            {/* Proof Points */}
            <div className="pf-label" id="pf-proof-label">
              Key Proof Points & Data Points
            </div>
              ...
              {proofPoints.length === 0 ? (
                ...
                proofPoints.map((p) => (
                  <span key={p} className="pf-tag" role="listitem">
                    ...
                    <button
                      className="pf-tag-x"
                      onClick={() => setProofPoints((prev) => prev.filter((x) => x !== p))}
                      aria-label={`Remove ${p}`}
                    >
                      ×
                    </button>
            ...
            <div className="pf-add-row">
              ...
              />
              <button className="pf-add-btn" onClick={addProofPoint}>
                Add
              </button>
            </div>

            {/* Preferred CTAs */}
            <div className="pf-label" id="pf-cta-label">
              Preferred CTA Styles / Phrases
            </div>
              ...
              {ctaPreferences.length === 0 ? (
                ...
                ctaPreferences.map((c) => (
                  <span key={c} className="pf-tag" role="listitem">
                    ...
                    <button
                      className="pf-tag-x"
                      onClick={() => setCtaPreferences((prev) => prev.filter((x) => x !== c))}
                      aria-label={`Remove ${c}`}
                    >
                      ×
                    </button>
            ...
            <div className="pf-add-row">
              ...
              />
              <button className="pf-add-btn" onClick={addCtaPreference}>
                Add
              </button>
            </div>

            {/* Preferred Structures */}
            <div className="pf-label" id="pf-structure-label">
              Preferred Formats / Structures
            </div>
              ...
              {preferredStructures.length === 0 ? (
                ...
                preferredStructures.map((s) => (
                  <span key={s} className="pf-tag" role="listitem">
                    ...
                    <button
                      className="pf-tag-x"
                      onClick={() => setPreferredStructures((prev) => prev.filter((x) => x !== s))}
                      aria-label={`Remove ${s}`}
                    >
                      ×
                    </button>
            ...
            <div className="pf-add-row">
              <input
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && (e.preventDefault(), addPreferredStructure())
                }
                aria-label="New preferred structure"
              />
              <button className="pf-add-btn" onClick={addPreferredStructure}>
                Add
              </button>
            </div>
                ...
                {bannedHashtags.length === 0 ? (
                  ...
                  bannedHashtags.map((t) => (
                    ...
                    >
                      ...
                      <button
                        className="pf-tag-x"
                        onClick={() => removeTag("ban", t)}
                        aria-label={`Remove ${displayTag(t)}`}
                        style={{
                          color: "color-mix(in srgb, var(--color-error-text) 50%, transparent)",
                        }}
                      >
            ...
            <div className="pf-add-row">
              ...
              />
              <button className="pf-add-btn" onClick={() => addTag("ban")}>
                Ban
              </button>
            </div>
            ...
            <div className="pf-add-row">
              ...
              />
              <button className="pf-add-btn" onClick={() => addTag("req")}>
                Require
              </button>
            </div>
            ...
            <div style={{ display: "flex", gap: 8, flexDirection: "column" }}>
              ...
              <div style={{ marginTop: 12 }}>
                ...
                <div className="pf-section-sub">
                  Your saved templates (saved from the wizard). Load a template to pre-fill the
                  wizard.
                </div>
                <TemplatesList />
              </div>
        ...
        {activeTab === "plan" && !profileLoading && <PlanSettings />}

        {activeTab === "api-keys" && !profileLoading && <ApiKeySettings />}
      </WorkspacePage>
    </>
  );
}
