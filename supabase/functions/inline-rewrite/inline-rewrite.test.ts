// F-002 regression: quota gating on inline-rewrite.
...
const { handleInlineRewrite } = await import("./index.ts");
...
function buildRequest(extra: Record<string, unknown> = {}): Request {
  return new Request("https://dummy.supabase.co/functions/v1/inline-rewrite", {
    method: "POST",
    headers: { Authorization: "Bearer dummy-jwt", "Content-Type": "application/json" },
    body: JSON.stringify({
      text: "Some selected text to rewrite.",
      instruction: "punchier",
      field: "body",
      platform: "LinkedIn",
      post: { title: "T", hook: "h", cta: "c" },
      ...extra,
    }),
  });
}

describe("inline-rewrite quota gating (F-002)", () => {
  beforeEach(() => {
    mockCheckQuota.mockReset();
    mockIncrement.mockClear();
    mockCallAIGateway.mockReset();
    mockCallAIGateway.mockResolvedValue(
      aiSuccess("return_rewrite", { rewrittenText: "Punchier rewritten text." })
    );
  });
  ...
  it("returns 402 QUOTA_EXCEEDED before any provider call when shared-key quota is exhausted", async () => {
    ...
    const res = await handleInlineRewrite(buildRequest());
  ...
  it("increments the generation count exactly once after a successful shared-key rewrite", async () => {
