// F-002 regression: quota gating on repurpose-post.
...
const { handleRepurposePost } = await import("./index.ts");
...
function buildRequest(extra: Record<string, unknown> = {}): Request {
  return new Request("https://dummy.supabase.co/functions/v1/repurpose-post", {
    method: "POST",
    headers: { Authorization: "Bearer dummy-jwt", "Content-Type": "application/json" },
    body: JSON.stringify({
      post: { title: "Src", hook: "h", body: "b", cta: "c", hashtags: "#x", dow: "Mon" },
      targetPlatform: "X",
      platform: "LinkedIn",
      ...extra,
    }),
...
describe("repurpose-post quota gating (F-002)", () => {
  beforeEach(() => {
    mockCheckQuota.mockReset();
    mockIncrement.mockClear();
    mockCallAIGateway.mockReset();
    mockCallAIGateway.mockResolvedValue(aiSuccess("return_post", POST_ARGS));
  });
  ...
  it("returns 402 QUOTA_EXCEEDED before any provider call when shared-key quota is exhausted", async () => {
    ...
    const res = await handleRepurposePost(buildRequest());
