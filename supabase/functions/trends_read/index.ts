...
export async function handler(req: Request) {
  ...
  if (!supabase)
    return new Response(JSON.stringify({ meta: { page, limit, total: 0 }, data: [] }), {
      status: 200,
    });
  ...
  let query = supabase
    ...
    .select(
      "id,title,normalized_terms,industry,platform,score,signal_count,first_seen,last_seen,metadata"
    )
